					...
					<Timeline defaultValue={1}>
						{appointments?.map((item, index) => (
							<TimelineItem key={item.id} step={index + 1} className="group-data-[orientation=vertical]/timeline:ms-10">
								...
								<TimelineContent className="mt-3 space-y-3 rounded-md border bg-muted text-accent-foreground p-3 sm:p-4">
									<div className="text-xs sm:text-sm ">
										<span className="font-medium">Date:</span> {dateToDOB(item.date)}
									</div>

									<div className="flex flex-col gap-2 text-xs sm:text-sm sm:flex-row sm:items-center sm:gap-6">
										<div>
