...
export const Appointments = ({
	...
	if (appointments?.length === 0) {
		return (
			<Card className="flex flex-col items-center justify-center space-y-4 px-4 py-20 text-center">
				<Ban className="h-12 w-12 text-gray-400" />
				<h3 className="text-lg font-semibold text-gray-700">
					No Appointments Found
				</h3>
				<p className="text-muted-foreground max-w-md text-sm">
					You currently have no {role === 'patient' ? 'upcoming' : 'scheduled'}{' '}
					appointments.
					...
					<Timeline defaultValue={1}>
						{appointments?.map((item, index) => (
							...
							>
								...
								<TimelineContent className="mt-4 space-y-4 rounded-md border bg-white p-4">
									<div className="text-sm text-gray-700">
										<span className="font-medium">Date:</span>{' '}
										{dateToDOB(item.date)}
									</div>

									<div className="flex flex-col gap-4 text-sm text-gray-700 sm:flex-row sm:items-center sm:gap-6">
										<div>
